        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6">
            ...
            {!user ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    >
                        POST COMMENT <IoIosSend className="text-base sm:text-lg" />
                    </button>
                </form>
            ) : (
                // If not logged in, show login prompt
                <div className="text-center text-gray-700">
                    <p className="mb-3 text-sm sm:text-base">Please login to leave a comment.</p>
                    <button
                        onClick={() => navigate("/auth/signin")}
                        className="bg-[#7E4555] text-white px-4 sm:px-6 py-2 sm:py-3 rounded-full hover:bg-[#B5697E] transition text-sm sm:text-base"
                    >
                        Login
                    </button>
                </div>
